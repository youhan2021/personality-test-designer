...
- **隐藏人格 HHHH**：24题全部选低分（value=1），触发"摆烂仙人"
- **隐藏人格 DRUNK**：24题全部选高分（value=3），触发"卷王之王"

### 英文字母编码规范

人格类型的英文字母代码必须能让用户**望码知义**，类似 MBTI 的设计思路。规则：

1. **优先使用真实英文单词/词根**，而非随机字母组合
2. **缩写要能在 Google/百度上搜到相关含义**
3. **同一类型的主旨和代码要吻合**

好的例子：
- `NOLEIF` = No Life（卷王/全天候）
- `GACHON` = Gacha On（抽卡狂人）
- `HOARD` = Hoard（囤积党）
- `MOD` = Discord Moderator（社区运营）
- `SENPAI` = 前辈（入坑引路人）
- `VIBEZ` = Vibes（摆烂/氛围感）
